...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      ...
      const response = await axios({
        ...
        data : {address : newAddress.value},
        headers:{Authorization : `Bearer ${localStorage.getItem("token")}`}
      })
      // console.log("New address getting registered")
      setAddresses({ ...addresses, all: response.data });
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      const IdSelected =  data.filter(item=>item._id === addresses.selected)
      const response = await axios.post(`${config.endpoint}/cart/checkout`,
      {addressId :IdSelected[0]._id},
      {headers:{Authorization : `Bearer ${token}`}}
      )
      enqueueSnackbar(
      ...
      if (e.response) {
        ...
        enqueueSnackbar(
          "Could not proceed to payment. Check that the backend is running, reachable and returns valid JSON.",
          {
            variant: "error",
          }
        );
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              <Box 
                ...
                key = {item._id}>
                <Typography color="#3C3C3C" my="0.5rem">
                  {item.address}
                </Typography>
                <Button 
                  variant="text" 
